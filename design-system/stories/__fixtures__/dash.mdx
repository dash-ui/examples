...
- **Tiny** ([< 6kB](https://bundlephobia.com/result?p=@dash-ui/styles@latest)), but
  comprehensive api
- **Bring your own UI framework** React, Preact, Vue, Svelte, etc.
- **Strongly typed** with TypeScript
- **CSS variables** are a first-class citizen
- **Themes** are easy and built with design tokens and selectors
- **_Fast_**, some may say blazing™
- **Autoprefixing** for vendor-specific styles
- **Nesting** `div { > * + * { margin-left: 4px; } }`
- **Minification**
- **Server rendering** [made easy](#server-rendering)
- **Flushable globals** make adding and removing global styles a breeze
- **Available as a [UMD](https://unpkg.com/browse/@dash-ui/styles@latest/dist/umd/) and [ES Module](https://cdn.skypack.dev/@dash-ui/styles/)**
